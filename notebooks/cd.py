...
def F(T, L, A=1, a=1/2, b = 1/2):
    ''' Cobb Douglas production Function'''
    return  A * T**a * L **b


def TC(Q, A=1, w=1, r=1, a=1/2, b = 1/2, F=0):
    ''' Cobb Douglas total cost function'''
    B = ( (b/a)**(a/(a+b)) + (a/b)**(b/(a+b)) )
    return B * (Q/A)**(1/(a+b))   *  w**(b/(a+b))  *  r**(a/(a+b))  + F

def AC(Q, A=1, w=1, r=1, a=1/2, b = 1/2, F=0):
    return TC(Q, A, w, r, a, b, F)/Q

def MC(Q, A=1, w=1, r=1, a=1/2, b = 1/2):
    B = ( (b/a)**(a/(a+b)) + (a/b)**(b/(a+b)) )
    return (1/(a+b)) * AC(Q, A, w, r, a, b, F=0)
